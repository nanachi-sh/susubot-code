...
type (
	...
	Users struct {
		Id                 string `db:"Id"`
		LastGetFortuneTime int64  `db:"LastGetFortuneTime"`
	}
)
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Id, data.LastGetFortuneTime)
	return ret, err
}

func (m *defaultUsersModel) Update(ctx context.Context, data *Users) error {
	query := fmt.Sprintf("update %s set %s where `Id` = ?", m.table, usersRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, data.LastGetFortuneTime, data.Id)
	return err
}
