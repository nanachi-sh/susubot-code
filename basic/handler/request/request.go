...
import (
	...
	"github.com/nanachi-sh/susubot-code/basic/handler/protos/fileweb"
	"github.com/nanachi-sh/susubot-code/basic/handler/protos/handler/request"
	request_d "github.com/nanachi-sh/susubot-code/basic/handler/request/define"
)
	...
	for _, v := range mc {
		...
		case request.MessageChainType_MessageChainType_Image:
			...
			if image.URL != nil {
				...
				resp, err := define.FilewebC.Upload(define.DefaultCtx, &fileweb.UploadRequest{
					Buf: image.Buf,
				})
				if err != nil {
					return nil, err
